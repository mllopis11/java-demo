package mike.demo.tasksched.module.core;

import java.util.concurrent.ThreadPoolExecutor;

public class SchedulerStats {

	private final String name;
	private final SchedulerState state;
	private final int minThreads;
	private final int maxThreads;
	private final int activeThreads;
	private final int idleThreads;
	private final int largestPoolSize;
	private final boolean terminating;
	
	protected SchedulerStats(String name, SchedulerState state, ThreadPoolExecutor executor) {
		this.name = name;
		this.state = state;
		this.minThreads = executor.getCorePoolSize();
		this.maxThreads = executor.getMaximumPoolSize();
		this.activeThreads = executor.getActiveCount();
		this.idleThreads = executor.getPoolSize() - this.activeThreads;
		this.largestPoolSize = executor.getLargestPoolSize();
		this.terminating = executor.isTerminating();
	}
	
	public String getName() {
		return name;
	}

	public SchedulerState getState() {
		return state;
	}

	public int getMinThreads() {
		return minThreads;
	}

	public int getMaxThreads() {
		return maxThreads;
	}

	public int getActiveThreads() {
		return activeThreads;
	}

	public int getIdleThreads() {
		return idleThreads;
	}

	public int getLargestPoolSize() {
		return largestPoolSize;
	}

	public boolean isTerminating() {
		return terminating;
	}

	@Override
	public String toString() {
		return String.format(
				"[%s::%s] (terminating: %s) threads={min=%d, max=%d, active=%d, idle=%d, largestPoolSize=%d}",
				name, this.state, terminating, minThreads, maxThreads, activeThreads, idleThreads, largestPoolSize);
	}

}
